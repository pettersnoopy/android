...
class PageTreeNode {
    ...
    private int getChildIndex(){
    	if(pageSliceBounds.contains(new RectF(0, 0, 0.5f, 0.5f))){
    		return 0;
    	}
    	if(pageSliceBounds.contains(new RectF(0.5f, 0, 1.0f, 0.5f))){
    		return 1;
    	}
    	if(pageSliceBounds.contains(new RectF(0, 0.5f, 0.5f, 1.0f))){
    		return 2;
    	}
    	if(pageSliceBounds.contains(new RectF(0.5f, 0.5f, 1.0f, 1.0f))){
    		return 3;
    	}
    	return 0;
    }
            ...
            public void decodeComplete(final Bitmap bitmap) {
                documentView.post(new Runnable() {
                    public void run() {
                        ...
                        if(documentView.creatingMap && isLastDecodeChild()){
                        	documentView.post(new Runnable() {
								@Override
								public void run() {
									documentView.convertAndSave(page);
								}
							});
                        	
//                        	documentView.saveBitmapToSdcard(bitmap, page.index);
                        }
                    }
                });
            }
    ...
    private boolean isLastDecodeChild(){
    	if(pageSliceBounds.bottom==1&&pageSliceBounds.right==1){
    		return true;
    	}
    	return false;
    }
